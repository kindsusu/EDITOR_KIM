...
setup 빌드는 EDITOR_KIM을 PDF 연결 프로그램으로 등록합니다. Windows의 "연결 프로그램" 목록에서 EDITOR_KIM을 고르면(또는 기본 프로그램으로 지정하면) PDF를 클릭해서 바로 열 수 있습니다 — 인터넷 임시 다운로드 위치에서 이렇게 연 파일은 배너가 뜨고 "다른 이름으로 저장"으로 저장됩니다.
...
**폰트 맞추기**는 그룹에 속하지 않은 텍스트 상자 하나의 한 줄(2,000자 이하)에 사용할 수 있습니다. 그림 위의 투명 검색 글자이거나 이미 폰트를 지정한 줄은 편집 확정(Enter) 시 이 창이 열립니다. 원본 폰트에 없는 글자는 예전처럼 맑은 고딕으로 바로 대체되며, 편집 패널의 **폰트 맞추기** 버튼으로 언제든 창을 열 수 있습니다. AI 후보 추천은 창 안의 버튼을 눌렀을 때만 실행되고, PDFium이 처리 가능한 글자는 AI를 호출하지 않습니다.
...
도구줄의 **페이지** 메뉴는 문서 단위 도구 5가지를 모읍니다(이미지 내보내기 외에는 모두 실행 취소 가능). **페이지 추출**은 체크한 페이지만 남기거나(축소본 체크박스 또는 `1,3-5` 범위 입력) 선택한 페이지를 삭제하고, **병합**은 여러 PDF를 고른 순서대로(현재 문서를 맨 앞에 포함 가능) 이어붙이고, **이미지로 내보내기**는 페이지를 96~300dpi PNG/JPEG로 폴더에 저장하고, **이미지 삽입**은 JPEG/PNG를 페이지에 넣어 드래그로 이동·손잡이로 크기 조절하고, **용량 줄이기**는 큰 이미지를 목표 dpi·품질 또는 목표 용량에 맞춰 다시 압축하며 투명 이미지는 건너뜁니다. (인터넷·메일에서 PDF를 클릭해 바로 여는 방법은 위 연결 프로그램 안내를 참고하세요.)
