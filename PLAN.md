...
### P2-W — 투명 글자(그림 위 검색용) 편집 (2026-09-04 사용자 보고: "HARMONY 인식은 되는데 수정이 안 됨")
- 원인: PowerPoint가 글자 효과를 그림으로 내보내면서 검색용으로 **채움 알파 0인 투명 글자**를 깔아 둠(해당 덱 1쪽 글자 객체 9개 모두, 이미지 1,045개). 편집은 성공하지만 투명 글자만 바뀌고 보이는 그림은 그대로
- [x] `objects()`에 `hidden`(알파 0 또는 렌더 모드 3/7). `_setOne`이 투명 글자를 고칠 때: 편집 전에 그 자리 배경색·글자색(`sampleInk`, 배경과 다른 픽셀의 최빈색)을 재고 → 배경색 사각형으로 그림을 덮고 → 글자를 렌더 모드 0·글자색·알파 255로 바꿔 맨 위에 올림. 새 idx를 돌려주고 setText/fitText가 따라감
- [x] UI: 투명 글자 상자는 회색 점선, 패널 안내, 적용 후 "그림을 덮고 새로 그렸습니다". 가리기 시 "그림은 덮기만" 경고
- 검수: 덱 제목 조각을 "ABC"로 → 원래 글자색(62,101,255)으로 그림 위에 그려짐. 시험 추가(알파 0 → 편집 → 진한 픽셀 158)
- 한계: 그림 글자의 배경이 그러데이션이면 덮은 자리가 단색으로 보임. 글자 폭이 그림보다 좁으면 옛 그림이 옆에 남을 수 있음(여백 3pt)

### P2-X — 대체 폰트 글자 깨짐 (2026-09-04 사용자 보고: 투명 글자를 고치니 글씨체가 완전히 깨짐)
- 원인 1: 투명 글자를 드러내며 알파 255로 바꿔도 **PDFium은 알파 1.0을 gs로 기록하지 않아** 저장·재열기(실행 취소 포함) 후 원래 ca 0이 되살아나 다시 투명해짐 → 알파 254로 설정해 명시적으로 기록되게
- 원인 2: 재열기한 문서에서 대체 폰트("Untitled") 객체를 제자리 SetText하면 PDFium이 임베드 폰트 대신 시스템 폰트로 대체 → 가늘고 벌어진 글자. 사용자 화면이 이것 → 폰트 이름이 Untitled면 제자리 SetText 대신 항상 새 서브셋 객체로 재생성
- 검수: 덱 자막을 고침 → 저장 → 재열기: 파란 굵은 글자 유지(알파 254) → 재편집: 같은 폰트. 시험에 재열기·재편집 폭 비교 추가
- 참고: 덱의 보이는 자막 폰트는 그림이라 알 수 없다. 대체 폰트는 맑은 고딕 굵게(원본 텍스트 폰트 Calibri-Bold의 굵기 따름)
- 사고: 진단 중 사용자가 띄워 둔 앱 서버(4747)에 요청을 보내 그 세션의 문서를 닫아 버림(미저장 편집 유실 가능). 진단은 반드시 별도 포트/프로세스로

### P2-Y — 투명 글자 편집 시 옛 글자가 남아 둘이 됨 (2026-09-04 사용자 보고: 나만의→너만의 했더니 둘이 됨)
- 원인: 덮는 사각형이 투명 텍스트 상자 크기(Calibri라 한글 폭 정보 없음 → 너무 작음)라 옛 그림이 안 덮임. PowerPoint는 글자 조각의 그림 타일을 옆 조각 자리까지 걸치게 넣어 단순히 타일을 합치면 옆 단어까지 덮음
- [x] `_inkExtent`: 겹치는 그림 타일 범위 안에서, 텍스트 상자 가운데부터 좌우로 잉크가 이어지는 구간만 찾아 그만큼만 덮음(빈 열이 글자 크기의 0.25 이상이면 멈춤). 옆 단어는 안 건드림
